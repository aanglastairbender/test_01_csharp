...
namespace JBC_PMIS_Government.ApplicationObjects.DataAccessObjects
{
    class SystemConfigurationDAO
    {
        ...
        public bool updateSoftwareInfo(string pValue)
        {
            bool _success = false;
            MySqlTransaction _myTrans;
            try
            {
                _myTrans = GlobalVariables.Connection.BeginTransaction();
                MySqlCommand _cmd = new MySqlCommand("call spUpdateSoftwareInfo('SoftwareInfo','" + pValue + "')", GlobalVariables.Connection);
                try
                {
                    _cmd.Transaction = _myTrans;
                    int _RowsAffected = _cmd.ExecuteNonQuery();
