...
namespace JBC_PMIS_Government.Global
{
    public partial class LookUpValueUI : Form
    {
        ...
        public void refreshFromSearch(string pViewType, string pSearchString)
        {
            try
            {
                loRecord.Rows.Clear();
                object[] _params = { pViewType, pSearchString };
                loRecord = (DataTable)lObject.GetType().GetMethod("getAllData").Invoke(lObject, _params);
                dgvLookUp.Rows.Clear();
                dgvLookUp.Columns.Clear();
                lCountCol = loRecord.Columns.Count;
        ...
        {
            if (this.dgvLookUp.Columns[e.ColumnIndex].Name == "Active" ||
                this.dgvLookUp.Columns[e.ColumnIndex].Name == "No." ||
                this.dgvLookUp.Columns[e.ColumnIndex].Name == "Bio. Id")
            {
                if (e.Value != null)
                {
